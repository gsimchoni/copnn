...
rossmann %>%
  filter(Store <= 5) %>%
  ggplot(aes(Date, Customers, col = factor(Store))) +
  geom_line()
...
rossmann <- rossmann %>%
  ...
  summarise(across(c(Open, Promo, SchoolHoliday,
                     starts_with("holiday")), sum), Customers = mean(Customers)) %>%
  ungroup() %>%
...
# Calculating 90th quantile
ross_stores <- ross_stores %>% inner_join(
  rossmann %>%
    group_by(Store) %>%
    summarise(Q9 = quantile(Customers, 0.9))
)

# Joining, creating binary y and sinking
rossmann %>%
  inner_join(ross_stores, by = "Store") %>%
  mutate(ExtremeMonth = ifelse(Customers > Q9, 1, 0)) %>%
  write_csv("rossmann_extreme.csv")
