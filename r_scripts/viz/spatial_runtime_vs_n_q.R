...
increase_n <- increase_n %>%
  mutate(exp_type = case_when(
    ...
    exp_type == "lmmnn" ~ "LMMNN",
    exp_type == "ohe" ~ "OHE",
    exp_type == "copnn" ~ "COPNN"
  ),
  exp_type = fct_relevel(exp_type, c("Ignore", "OHE", "Embed.", "LMMNN", "COPNN")))
...
pN <- increase_n %>%
  ...
  scale_x_continuous(breaks = log10(c(10000, 100000, 1000000, 10000000)),
                     labels = c(expression(10^4), expression(10^5), expression(10^6), expression(10^7))) +
  ...
  scale_color_manual("", values=c("brown", "purple", "orange", "red", "blue")) +
